...
Other languages like Python and Kotlin require the implementation of special methods as part of the class in order for them to be exposed as operators. For example, we can achieve the above result in Kotlin [as](https://pl.kotl.in/XkrlBsH0F):
...
While this approach looks okay to begin with, string formatting becomes increasingly complex. Compare that with plain old `printf`:
